...
class Tablefield extends FormElementBase {
  ...
  public static function processTablefield(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    $weightedRows = [];
    for ($i = 0; $i < $rows; $i++) {
      $key = (isset($table_keys[$i]) && is_numeric($table_keys[$i])) ? $table_keys[$i] : $i;
      $weight = $table[$key]['weight'] ?? $i;
      $weightedRows[$key] = [
      ...
      $draggable = TRUE;
      for ($ii = 0; $ii < $cols; $ii++) {
        if (!empty($element['#locked_cells'][$key][$ii]) && !empty($element['#lock'])) {
          $draggable = FALSE;
          $weightedRows[$key][$ii] = [
            '#type' => 'item',
            '#value' => $element['#locked_cells'][$key][$ii],
            '#title' => $element['#locked_cells'][$key][$ii],
          ];
        }
        else {
          $cell_value = isset($value[$key][$ii]) ? $value[$key][$ii] : '';
          $weightedRows[$key][$ii] = [
            '#type' => $input_type,
            '#maxlength' => 2048,
            '#size' => 0,
      ...
      if ($draggable) {
        $weightedRows[$key]['#attributes']['class'][] = 'draggable';
      }
    }
