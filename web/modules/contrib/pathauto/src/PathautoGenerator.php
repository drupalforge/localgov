...
class PathautoGenerator implements PathautoGeneratorInterface {
  ...
  public function updateEntityAlias(EntityInterface $entity, $op, array $options = []) {
    ...
    // Skip the entity is syncing, for example during migrations or workspace
    // publishing.
    if ($entity instanceof SynchronizableInterface && $entity->isSyncing()) {
      return NULL;
    }

    // Only act if this is the default revision or if the entity is updated in a
    // workspace.
    $skip_revisionable = $entity instanceof RevisionableInterface && !$entity->isDefaultRevision();
    if ($this->moduleHandler->moduleExists('workspaces') && \Drupal::service('workspaces.manager')->hasActiveWorkspace()) {
      $skip_revisionable = FALSE;
    }
    if ($skip_revisionable) {
      return NULL;
    }
