...
interface GeonamesTimezoneInterface {
   ...
   *   timezoneId: name of the timezone (according to olson),
   *     this information is sufficient to work with the timezone and defines
   *     DST rules, consult the documentation of your development environment.
   *     Many programming environments include functions based on the olson
   *     timezoneId (example java TimeZone)
   *   time: the local current time
   *   sunset: sunset local time (date)message:
   *   sunrise: sunrise local time (date)
   *   rawOffset: the amount of time in hours to add to UTC to get standard time
   *     in this time zone. Because this value is not affected by daylight
   *     saving time, it is called raw offset.
